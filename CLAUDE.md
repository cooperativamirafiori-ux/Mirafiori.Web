...
| **Costi strutture** | `app/(app)/inserisci-costo/` `cruscotto-costi/` | `app/api/costi/` | `lib/costi/data.ts` | il cruscotto ha due viste: per centro di costo (default) e per struttura<br>⚠️ **`creaCosto()` scrive in due posti** (7 set 2026): la lista SP *e* il registro analitico su Supabase. Il riversamento **non fa fallire** l'operazione se Supabase non risponde — il costo su SP è la cosa che conta, e farlo fallire farebbe reinserire il costo creando il doppione vero. Il buco si richiude con `scripts/travaso-costi-registro.mjs`, che è ripetibile (`origine_id` = id item SP) ed è anche lo strumento di **quadratura** fra lista e registro<br>la categoria si traduce in voce analitica via la tabella `mappa_categoria_voce`, non da una costante: `Categoria` su SP è testo libero |
...
| **Richiesta fattura** | `app/(app)/richiesta-fattura/`: **modulo a passi** — `RichiestaFatturaForm.tsx` tiene stato e navigazione, un file per passo in `_componenti/`, l'elenco dei passi e di chi è ogni campo in `_componenti/passi.ts` | `app/api/fatture/`<br>`app/api/clienti/[id]/`<br>`app/api/clienti/partita-iva/[numero]/` (VIES) | `lib/fatture/`: `data.ts` `notifiche.ts` `centri-di-costo.ts`<br>anagrafica: `lib/clienti/data.ts` `lib/clienti/vies.ts`<br>campi e validazione in `types/fatture.ts` · comuni in `types/comuni.ts` | aperta a **tutti** gli utenti, nessun permesso d'area<br>⚠️ **pensata anche per anziani e persone con qualche difficoltà** (24 set 2026): una domanda per schermata, bottoni grandi, parole semplici. Una modifica che aggiunge un campo va chiesta con la stessa cura, non messa in fondo a un passo<br>**nuovo cliente dal QR** (25 set 2026): pagina **pubblica** `app/nuovo-cliente/` → `app/api/nuovo-cliente/` → `lib/clienti/pubblico.ts`; regole in `types/nuovo-cliente.ts` (riusa `validaRichiesta`); non restituisce mai dati dell'anagrafica; niente doppioni: P.IVA **o** CF già presenti → chiede conferma e aggiorna; nessuna mail a nessuno (scelta di Dennis); informativa in `app/privacy/clienti/` (testo INF03 v3.3)<br>elenco comuni in `public/comuni.json` ← `scripts/aggiorna-comuni.mjs`<br>`scripts/provision-fatture.mjs` `provision-clienti.mjs` `import-clienti.mjs`<br>`docs/richiesta-fattura.md` |
...
| **Amministrazione · Password** | `app/(app)/amministrazione/password/` (`GestionePassword.tsx`) | `app/api/password/**` | `lib/password/data.ts` | cassaforte condivisa delle credenziali, cancello = permesso `Amministrazione`<br>⚠️ **password e PIN in chiaro sulla lista SP** (scelta di Dennis, 15 set 2026): `leggiSegreto`/`scriviSegreto` in `data.ts` sono le uniche due porte, il giorno che si cifra si toccano solo quelle<br>⚠️ `UltimaModificaPassword` si muove **solo se la password cambia davvero**: per questo `aggiornaVocePassword` rilegge la riga prima di scriverla<br>nel Log Attività non finisce mai un valore riservato, solo il nome della voce<br>`scripts/provision-password.mjs` · `docs/gestione-password.md` |
| **Controllo di Gestione · Flussi fatture** | `app/(app)/controllo-gestione/` (hub) + `flussi-fatture/` (`FlussiFatture.tsx`, `Testata.tsx`, `NuovaUscita.tsx`, `IbanRiga.tsx`) | `app/api/pagamenti/**`, `app/api/cron/fatture-sdi` | `lib/pagamenti/`: `sdi/` (import XML) `verifica.ts` (da verificare, IBAN) `data.ts` `flusso.ts` `guard.ts` `uscite.ts`; `xlsx.ts` `tracciato.ts` `import.ts` = vecchio import Excel | dati su **Supabase** (`supabase/pagamenti_schema.sql` + `uscite_manuali.sql` + `fatture_sdi.sql`)<br>**le fatture arrivano dagli XML SDI** nella cartella SP `General/fatture da SDI` (cron notturno + "Importa adesso"), non più dall'Excel: `docs/fatture-sdi.md`<br>⚠️ **una fattura già pagata non deve MAI finire in coda**: nel dubbio `da_verificare`<br>⚠️ i nomi file SDI **distinguono maiuscole e minuscole**<br>**Fatture del servizio** (`controllo-gestione/fatture/`, `app/api/centri-costo/fatture`, `lib/pagamenti/assegnazione.ts`): il coordinatore segna le fatture libere sul suo CC, vince il primo, stesso accesso della scheda Qonto<br>⚠️ l'export di Fattura SMART **non si legge con exceljs** (namespace `x:`, celle senza `r=`): vedi `lib/pagamenti/xlsx.ts`<br>⚠️ **il join sulla fattura in `data.ts` è esterno, senza `!inner`**: con `!inner` sparirebbero senza errore tutte le uscite senza fattura<br>le **uscite fuori SDI** (F24, tributi, rate) sono scadenze con `fattura_passiva_id` nullo nella stessa tabella — stesse code, stesso PAGATA, stessi totali; nascono `da_pagare` e non passano dalla soglia: `docs/uscite-senza-fattura.md`<br>**tre permessi**: `Controllo di Gestione` (cruscotti), `Pagamenti` (paga, carica, inserisce a mano), `Approvazione Pagamenti` (approva) — la sezione si apre con uno qualsiasi<br>soglia dalla lista SP Parametri (`SogliaApprovazionePagamenti`)<br>`docs/flussi-fatture.md` · piano completo in `docs/controllo-di-gestione-piano.md` |
| **Controllo di Gestione · Qonto** | `app/(app)/controllo-gestione/qonto/` (elenco conti + `[id]/` saldo, IBAN, ultimi movimenti) | — (pagine server, nessuna API) | `lib/qonto/`: `client.ts` (solo GET, chiave API, cache 60 s) `data.ts` `accesso.ts` `formato.ts` | **un sottoconto Qonto per centro di costo**, riconosciuto dal prefisso del nome `ccN · …` (il codice non si tocca, il resto sì)<br>**chi vede cosa** in `lib/qonto/accesso.ts`: permesso `Controllo di Gestione` → tutti i conti, principale compreso; **coordinatore** del CC (lista SP Centri di Costo, colonna interna `Responsabile`, visibile "Coordinatori", multipla) → solo i suoi, **senza permesso da assegnare**. I coordinatori entrano in Controllo di Gestione anche senza permessi e vedono solo la scheda Qonto<br>⚠️ **l'app legge con la chiave API**. L'unica scrittura sono le **richieste di bonifico** ("Invia a Qonto" in Flussi fatture, `lib/qonto/bonifici.ts`), che Qonto accetta **solo con OAuth** (401 con la chiave, provato): token in Supabase `qonto_token`, rinnovo a turno in `lib/qonto/oauth.ts` perché il refresh token è **monouso**; consegnato dal Mac con `scripts/qonto-oauth-a-supabase.mjs` (che rinomina il file locale: Mac e app non devono usare lo stesso token). Le richieste risultano a nome di Dennis, le approva Claudia su Qonto. Giro notturno: tiene vivo il token e segna pagate le richieste approvate. `docs/qonto-bonifici.md`<br>⚠️ **la documentazione Qonto sbaglia sulla chiave API** per alcune scritture (rinomina sottoconto → 401): provare, non fidarsi<br>⚠️ **dalla sandbox Qonto non si raggiunge** (proxy 403): gli script Qonto li lancia Dennis<br>coordinatori: `scripts/coordinatori-centri-costo.mjs` · `docs/qonto.md` |
| **Controllo di Gestione · Registro** | — (ancora nessuna schermata) | — | `lib/gestione/registro.ts` | il registro analitico su **Supabase** (`supabase/gestione_schema.sql`): `movimento` più anagrafiche, cattura, banca, costo del lavoro, budget<br>⚠️ **si scrive solo da `scriviDocumento()`**, che chiama la funzione SQL `registro_riscrivi()`: un insert diretto può lasciare un documento contato due volte, perché riattribuire vuol dire togliere una riga e metterne un'altra e le due chiavi sono diverse<br>⚠️ **costo = importo negativo, ricavo = positivo**, imposto da vincolo; nei report i costi tornano positivi<br>⚠️ `DA_ATTRIBUIRE` è una riga vera dell'anagrafica, non un NULL: con `cc_codice` nullable il vincolo anti-doppio-conteggio non funzionerebbe<br>le aggregazioni sono funzioni SQL (`registro_per_cc`, `registro_per_voce`): PostgREST non fa GROUP BY<br>anagrafica CC in specchio da SharePoint: `scripts/sync-centri-costo-supabase.mjs`<br>`docs/registro-fondamenta.md` · piano completo in `docs/controllo-di-gestione-piano.md` |
